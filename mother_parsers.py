...
class Mother():
    ...
    def __init__(self, id_num):
        self.id_num = id_num
        self.df_subset = Mother.master_df.loc[Mother.master_df['caseid'] == id_num]                 # the relevant rows of the df
        self.dhs_clust = self.df_subset['clusterno'].iloc[0]
        self.birth_year = self.df_subset['birthyear'].iloc[0]
        self.collected_range = list(range(self.birth_year + 15, Mother.collection_year + 1))        # over what years was this mother surveyed
...
def main():
    ...
    id_num_list = input_df['caseid'].tolist()
    ...
    df = pd.DataFrame(data=data, columns=['DHSCLUST', 'Mother ID#', 'Year', 'Mother\'s Age', 'Baby?'])

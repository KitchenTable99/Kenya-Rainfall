...
def distances(file_path, station_coords, cmd_args, testing):
    '''Finds distances between station and the rainfall center.
    ...
    Returns:
        tuple: a tuple containing a list of all the distances and the GeoPandas DataFrame in that order
    '''
    # import shapefile
...
def shapeFileParser(file_path, station_coords, cmd_args, testing=False):
    ...
    '''
    alldist, gdf = distances(file_path, station_coords, cmd_args, testing=cmd_args.testing)
    if cmd_args.distance == False:
        min_dist = dd.body(deepcopy(alldist), cmd_args)
        cmd_args.distance = statistics.quantiles(min_dist)[2]
        print(f'\nThe distance used to calculate rainfall data is {cmd_args.distance} km.\n')
    # create a new column and assign it the relevant station indices
    monitor_stations = [[index for index, dist in enumerate(row) if dist <= cmd_args.distance] for row in progress(alldist, desc='Finding nearby stations')]
